...
ggplot(res, aes(x = DateTimeStamp, y = NEM)) +
  ...
  labs(
    x = NULL,
    y = 'Net Ecosystem Metabolism (mmol O2 m-2 d-1)'
  )
```

Or we can average NEM daily and plot it again. 
...
## Using EBASE with long time seriers

Execution time of the model can also be reduced by using multiple processors.  This is done using [doParrallel](https://cran.r-project.org/package=doParallel) package and registering a parallel backend as below. It doesn't really help us when the time series is short, but it can be useful when running more than a few days of data.
...
res <- ebase(apadb, interval = 3600, Z = zval)
...
The `doave` argument can be used to define which dissolved oxygen value is used as the starting point in the Bayesian estimation for the optimization period.  The default setting (`doave = TRUE`) will use the average of all the dissolved oxygen values in the optimization period defined by `ndays`.  The first dissolved oxygen observation of the time series in the optimization period will be used as the starting point if `doave = F`.  In this case, the modeled dissolved oxygen time series will always start at the first observed dissolved oxygen value for each optimization period. 

Below, `doave = FALSE` is used to set the first observed dissolved oxygen as the starting value.

```{r}
res <- ebase(apadbsub, interval = 3600, Z = zval, doave = F)
```
...
The `ebase_prep()` function is used internally to `ebase` to prepare the data for the metabolism calculations.  This function interpolates the missing data and returns a column `isinterp` that specifies `TRUE` or `FALSE` if a value is interpolated.  We'll use this function to demonstrate how the interpolation works.  

```{r}
#| warning: false
dat2interp <- ebase_prep(apadbmiss, interval = 3600, Z = zval)
head(dat2interp)
```
...
#| fig-height: 3
#| fig-width: 9
interp_plot(dat2interp, Z = zval, interval = 3600, param = 'DO_sat')
```
