...
Here, the prior distribution for the $b$ parameter is fixed to 0.251 (cm h$^{-1}$)/(m$^2$ s$^{-2}$), as suggested by @Wanninkhof14.  We can view this change with the `prior_plot` function before using `ebase()`. Note that we define the mean as our desired value and assign a very small range for the standard deviation.
