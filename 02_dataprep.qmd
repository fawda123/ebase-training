...
The data are in the correct format, but the column names are not.  We need to change the names, convert the date/time column to a POSIXct object with the correct time zone, and convert the PAR data to the correct units.  PAR is micromoles per square meter per second, so we need to convert this to Watts per square meter.  First, we select and rename the columns.
...
Next, we use mutate to convert the date/time column to a POSIXct object and the PAR data to the correct units. For date/time, we can use a function from the lubridate package and use the "America/Jamaica" time zone, which is eastern time without daylight savings.  The PAR is converted using the conversion from above, except we don't need to convert from millimoles to micromoles and from 15 minutes to seconds. 
...
Next we import the weather data.  These can be similarly downloaded using the NOAA Tides & Currents [API](https://api.tidesandcurrents.noaa.gov/api/prod/#units).
...
We only need to select and rename the columns and convert the date/time column to a POSIXct object with the correct time zone.  Instead of `ymd_hms()` as above, we'll use `ymd_hm()` because seconds are not included. We'll do this in one step.
