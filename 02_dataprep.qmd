...
First we need to rename the columns and select those we want to make `apadb` look more like `exdat`.  This can all be done with the `select` function in dplyr.
...
We can verify the column names are the same between the two datasets.  Note the use of two equal signs - this is how we tell R to test for equality.
